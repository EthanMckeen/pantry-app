...
exports.LangTraceExporter = void 0;
const langtrace_exporter_1 = require("../../constants/exporter/langtrace_exporter");
const api_1 = require("@opentelemetry/api");
const axios_1 = __importDefault(require("axios"));
class LangTraceExporter {
    /**
     *
    ...
    constructor(apiKey, apiHost) {
        this.apiKey = apiKey;
        this.apiHost = apiHost === langtrace_exporter_1.LANGTRACE_REMOTE_URL ? `${langtrace_exporter_1.LANGTRACE_REMOTE_URL}/api/trace` : apiHost;
    }
    async export(spans, resultCallback) {
        // If the API key is not set, we will not export the spans
        if (this.apiHost === langtrace_exporter_1.LANGTRACE_REMOTE_URL && this.apiKey.length === 0) {
            await this.shutdown();
        }
        ...
        await axios_1.default.post(`${this.apiHost}`, data, { headers }).then((response) => {
            api_1.diag.info(`Exported ${spans.length} spans to ${this.apiHost} with status code ${response.status}`);
...
exports.LangTraceExporter = LangTraceExporter;
//# sourceMappingURL=langtrace_exporter.js.map